...
class Presentations(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(200), nullable=True)  # New optional field
    venue = db.Column(db.String(200), nullable=True)  # New optional field
...
def update_profile_pic():
    if "user" in session and session["user"] == params["user-name"]:
        ...
        if pic and allowed_file(pic.filename):
            ...
            if os.path.exists(
                os.path.join(app.config["UPLOAD_FOLDER"], "profile_pic.png")
            ):
                os.remove(os.path.join(app.config["UPLOAD_FOLDER"], "profile_pic.png"))
            pic.save(os.path.join(app.config["UPLOAD_FOLDER"], "profile_pic.png"))
...
def get_details():
    if request.json["accesskey"] == params["accesskey"]:
        ...
        details = (
            Details.query.filter_by(category=category_key)
            .order_by(desc(Details.id))
            .all()
        )
        final_data = []
        for det in details:
...
def education():
    if "user" in session and session["user"] == params["user-name"]:
        education = (
            Details.query.filter_by(category="education")
            .order_by(desc(Details.id))
            .all()
        )
        position = (
            Details.query.filter_by(category="position")
            .order_by(desc(Details.id))
            .all()
        )
        awards = (
            Details.query.filter_by(category="awards").order_by(desc(Details.id)).all()
        )
        members = (
            Details.query.filter_by(category="members").order_by(desc(Details.id)).all()
        )
        ...
        if "image" in request.files:
            ...
            if image.filename != "":
                if image and allowed_file(image.filename):
                    ...
                    filename = "_".join([basename, suffix])
                    filename_image = (
                        filename + "." + image.filename.rsplit(".", 1)[1].lower()
                    )
                    image.save(
                        os.path.join(app.config["UPLOAD_FOLDER"], filename_image)
                    )
                    publication.image_filename = filename_image
                    publication.image = url_for(
                        "static", filename="upload/" + filename_image
                    )
...
def publication():
    if "user" in session and session["user"] == params["user-name"]:
        publications = (
            Publication.query.filter_by().order_by(desc(Publication.id)).all()
        )
...
@app.route("/get-presentations", methods=["POST"])
def get_presentations():
    if request.json["accesskey"] == params["accesskey"]:
        presentations = (
            Presentations.query.filter_by().order_by(desc(Presentations.id)).all()
        )
        data = []
        for pres in presentations:
            dic = {
                "id": pres.id,
                "title": pres.title,  # New field
                "venue": pres.venue,  # New field
                "description": pres.description,
                "date": pres.date,
            }
            data.append(dic)
        return jsonify({"error": False, "data": data})
    return jsonify({"error": True, "data": "Invalid access key"})
...
def presentations():
    if "user" in session and session["user"] == params["user-name"]:
        ...
        return render_template("adminPresentations.html", presentations=presentations)
    return redirect(url_for("login"))
...
def add_presentation():
    if "user" in session and session["user"] == params["user-name"]:
        ...
        presentation.title = request.form.get("title")  # New field
        presentation.venue = request.form.get("venue")  # New field
        presentation.description = request.form.get("description")
        presentation.date = request.form.get("date")
...
@app.route("/update-presentation", methods=["POST"])
def update_presentation():
    if "user" in session and session["user"] == params["user-name"]:
        id = request.form.get("id")
        presentation = Presentations.query.filter_by(id=id).first()
        presentation.title = request.form.get("title")  # New field
        presentation.venue = request.form.get("venue")  # New field
        presentation.description = request.form.get("description")
        presentation.date = request.form.get("date")
...
@app.route("/delete-presentation", methods=["POST"])
